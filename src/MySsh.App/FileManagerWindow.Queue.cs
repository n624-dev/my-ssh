...
internal sealed partial class FileManagerWindow
{
    ...
    private void RetryOverwrite(TransferJobSnapshot job)
    {
        if (MessageBox.Query(70, 8, "Overwrite",
                ...
                "Overwrite", "Back") == 0)
            _transfers.Retry(job.Id, ConflictAction.Overwrite);
    }
    ...
    private void RenameTransferDestination(TransferJobSnapshot job)
    {
        ...
        {
            MessageBox.ErrorQuery(75, 8, "Rename Transfer", ex.Message, "OK");
        }
    }
    ...
    private void RefreshQueue()
    {
        ...
        if (display.Count > 0)
            _queueList.SelectedItem = Math.Clamp(selected, 0, display.Count - 1);

        var newlyCompleted = _queueRows
            .Where(x => x.State is TransferState.Completed or TransferState.Partial)
            .Where(x => _reloadedCompleted.Add(x.Id))
            .Any();
        ...
        {
            // A transient browser refresh failure must not change the transfer result.
        }
    }

    private static string QueueText(TransferJobSnapshot job)
    {
        var progress = job.TotalBytes is > 0
            ? $" {job.BytesTransferred * 100.0 / job.TotalBytes:0.0}%"
            : "";
        var speed = job.BytesPerSecond is > 0
            ? $" {FormatSize((long)job.BytesPerSecond.Value)}/s"
            : "";
        var eta = job.BytesPerSecond is > 0 && job.TotalBytes is { } total && total > job.BytesTransferred
            ? $" ETA {FormatDuration(TimeSpan.FromSeconds(
                (total - job.BytesTransferred) / job.BytesPerSecond.Value))}"
            : "";
        return $"{job.State,-9} {Safe(PathLikeName(job.Source))} -> " +
               $"{Safe(job.Destination)}{progress}{speed}{eta} {job.Message}";
    }
}
