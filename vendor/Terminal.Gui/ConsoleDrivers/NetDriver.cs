...
namespace Terminal.Gui {
	...
	internal class NetEvents {
		...
		ConcurrentQueue<InputResult> inputResultQueue = new ConcurrentQueue<InputResult> ();
		...
		public InputResult? ReadConsoleInput ()
		{
			while (!TokenSource.IsCancellationRequested) {
				...
				if (inputResultQueue.Count == 0) {
					inputReady.Wait (TokenSource.Token);
					inputReady.Reset ();
				}
#if PROCESS_REQUEST
				neededProcessRequest = false;
#endif
				if (inputResultQueue.TryDequeue (out var result)) {
					return result;
				}
			}
		...
		{
			while (!TokenSource.IsCancellationRequested) {
				waitForStart.Wait (TokenSource.Token);
				waitForStart.Reset ();
		...
		{
			while (!TokenSource.IsCancellationRequested) {
				winChange.Wait (TokenSource.Token);
				winChange.Reset ();
				WaitWinChange ();
				inputReady.Set ();
			}
...
#pragma warning restore CA1416
	...
	internal class NetMainLoop : IMainLoopDriver, IDisposable {
		...
		ConcurrentQueue<NetEvents.InputResult> inputResult = new ConcurrentQueue<NetEvents.InputResult> ();
		MainLoop mainLoop;
		CancellationTokenSource tokenSource = new CancellationTokenSource ();
		internal NetEvents netEvents;
		readonly Func<CancellationToken, NetEvents.InputResult?> readInput;
		internal Task InputTask { get; private set; }
		...
		public NetMainLoop (ConsoleDriver consoleDriver = null, Func<CancellationToken, NetEvents.InputResult?> readInput = null)
		{
			if (consoleDriver == null) {
				throw new ArgumentNullException ("Console driver instance must be provided.");
			}
			if (readInput == null) {
				...
				this.readInput = _ => netEvents.ReadConsoleInput ();
			} else {
		...
		void NetInputHandler ()
		{
			try {
				while (!tokenSource.IsCancellationRequested) {
					waitForProbe.Wait (tokenSource.Token);
					waitForProbe.Reset ();
					if (inputResult.IsEmpty) {
						var result = readInput (tokenSource.Token);
						if (result.HasValue) inputResult.Enqueue (result.Value);
					}
					if (!inputResult.IsEmpty) {
						keyReady.Set ();
					}
				...
				// Both the main-loop wait and the console-reader wait wake on shutdown.
			}
		}
		...
		void IMainLoopDriver.Setup (MainLoop mainLoop)
		{
			...
			InputTask = Task.Run (NetInputHandler, tokenSource.Token);
		}
		...
		bool IMainLoopDriver.EventsPending (bool wait)
		{
			...
			if (!tokenSource.IsCancellationRequested) {
				return !inputResult.IsEmpty || CheckTimers (wait, out _);
			}
		...
		{
			while (inputResult.TryDequeue (out var result)) {
				ProcessInput?.Invoke (result);
			}
		}
